...
flowchart TD
    ...
    F -- Yes --> G[dos_append::find_absolute_path]
...
bool DOS_OpenFile(const char* name, uint16_t flags, uint16_t* entry, bool fcb)
{
	...
	if (Files[handle]) {
		...
		if (!dos_append::IsResolving() && dos_append::IsEnabled()) {
			std::string resolved = {};
			if (dos_append::find_absolute_path(name, resolved)) {
...
#### Why it exists:
* **Primary Interception Point:** `DOS_OpenFile` is the primary workhorse of DOS file I/O. When a DOS game calls `open("CONFIG.DAT")` or `fopen("C:\GAME\DATA.PAK")` and the file is not at that literal path, `find_absolute_path()` searches each appended directory in order to construct the full drive path.
* **Secondary Order Rule:** `find_absolute_path()` is only called in the `else` branch (after literal path opening fails). This guarantees that files in the current working directory always take priority over appended directories.
...
* **Consistency:** If `DOS_GetFileAttr("DATA.PAK")` returned `FILE_NOT_FOUND` while `DOS_OpenFile("DATA.PAK")` succeeded, programs would assume a file system error and crash. Calling `find_absolute_path()` ensures metadata queries match file open operations.
...
## 3. Why File Creation & Deletion Exclude `find_absolute_path`

`dos_append::find_absolute_path` is **intentionally omitted** from `DOS_CreateFile` (Line 800) and `DOS_UnlinkFile` (Line 975):

```cpp
// DOS_CreateFile in dos_files.cpp
bool DOS_CreateFile(const char* name, uint16_t attribute, uint16_t* entry)
{
    // NO find_absolute_path call! Files are ALWAYS created at the target path.
    // ...
}
```
...
| DOS File API Function | INT 21h Subfunction | Uses `find_absolute_path`? | Rationale |
| :--- | :--- | :--- | :--- |
