...
protected:
	void SetUp() override
	{
		...
		Drives[2] = std::make_shared<localDrive>(
		        "tests/files/append/", 512, 1, 1, 1, 1, false);
		DOS_SetDefaultDrive(2);

		// MCB header is 1 paragraph (16 bytes) before the payload segment
		constexpr uint16_t MCB_HEADER_SIZE_PARAGRAPHS = 1;

		// ASCII 'M': non-terminal heap block
		constexpr uint8_t MCB_TYPE_MIDDLE_BLOCK = 0x4D;

		// PSP allocation parameters: Physical address 0x20000 (128 KB)
		constexpr uint16_t TEST_PSP_SEGMENT = 0x2000;

		// 256 bytes standard PSP structure
		constexpr uint16_t PSP_STRUCT_SIZE_PARAGRAPHS = 0x10;

		// 32 bytes trailing alignment padding
		constexpr uint16_t PSP_PADDING_PARAGRAPHS = 2;
		constexpr uint16_t PSP_TOTAL_ALLOC_PARAGRAPHS =
		        PSP_STRUCT_SIZE_PARAGRAPHS + PSP_PADDING_PARAGRAPHS;

		// Environment segment parameters placed directly after PSP payload (Segment 0x2012)
		constexpr uint16_t TEST_ENV_SEGMENT = TEST_PSP_SEGMENT + PSP_TOTAL_ALLOC_PARAGRAPHS;

		// 4,096 bytes environment buffer
		constexpr uint16_t ENV_BUFFER_SIZE_PARAGRAPHS = 0x100;

		// Initialize MCB header for test PSP
		DOS_MCB psp_mcb(TEST_PSP_SEGMENT - MCB_HEADER_SIZE_PARAGRAPHS);
		psp_mcb.SetPSPSeg(TEST_PSP_SEGMENT);
		psp_mcb.SetSize(PSP_TOTAL_ALLOC_PARAGRAPHS);
		psp_mcb.SetType(MCB_TYPE_MIDDLE_BLOCK);

		// Initialize MCB header for test environment block (owned by test PSP)
		DOS_MCB env_mcb(TEST_ENV_SEGMENT - MCB_HEADER_SIZE_PARAGRAPHS);
		env_mcb.SetPSPSeg(TEST_PSP_SEGMENT);
		env_mcb.SetSize(ENV_BUFFER_SIZE_PARAGRAPHS);
		env_mcb.SetType(MCB_TYPE_MIDDLE_BLOCK);

		// Zero-terminate environment block and bind environment pointer to PSP
		mem_writeb(PhysicalMake(TEST_ENV_SEGMENT, 0), 0);
		DOS_PSP psp(TEST_PSP_SEGMENT);
		psp.MakeNew(0);
		psp.SetEnvironment(TEST_ENV_SEGMENT);
		dos.psp(TEST_PSP_SEGMENT);
...
// Requirement: Verify initial default state is disabled with empty directory
// list. Target: dos_append::IsEnabled(), dos_append::GetDirectories()
...
// Requirement: Verify setting non-empty directory list enables APPEND
// resolution. Target: dos_append::SetDirectories(), dos_append::IsEnabled()
...
// Requirement: Verify setting empty directory list disables APPEND resolution.
// Target: dos_append::SetDirectories(), dos_append::IsEnabled()
...
// Requirement: Verify consecutive APPEND commands replace previous directory
// list. Target: dos_append::SetDirectories(), dos_append::GetDirectories()
...
// Requirement: Verify raw trailing directory backslashes are preserved in list
// storage. Target: dos_append::SetDirectories(), dos_append::GetDirectories()
...
// Requirement: Verify semicolon argument clears directory list. Target:
// APPEND::Run(), dos_append::SetDirectories()
...
// Requirement: Verify duplicate directory entries in command string are
// preserved. Target: APPEND::Run(), dos_append::GetDirectories()
...
// Requirement: Verify option switches are stripped from directory list parsing.
// Target: APPEND::Run(), parse_options()
...
// Requirement: Verify /X:ON and /PATH:OFF flags set corresponding subsystem
// flags. Target: dos_append::SetFlags(), IsExecOn(), IsPathOverrideOn()
...
// Requirement: Verify whitespace and surrounding quotes are trimmed from paths.
// Target: dos_append::validate_directories()
...
// Requirement: Verify empty token strings between multiple semicolons are
// ignored. Target: dos_append::validate_directories()
...
// Requirement: Verify relative directory paths expand to absolute DOS drive
// paths. Target: dos_append::validate_directories()
...
// Requirement: Verify invalid directory input aborts without mutating existing
// list. Target: dos_append::validate_directories()
...
// Requirement: Verify optional leading '=' sign is stripped during command
// parsing. Target: dos_append::validate_directories()
...
// Requirement: Verify path resolution returns false when APPEND list is empty.
// Target: dos_append::find_absolute_path()
...
// Requirement: Verify basename extraction resolves simple filenames against
// appended directories. Target: dos_append::find_absolute_path()
TEST_F(DosAppendTest, ResolveNameBasenameExtraction)
{
	...
	EXPECT_FALSE(dos_append::find_absolute_path("D:\\OTHER\\README.TXT",
	                                            out_path));
...
// Requirement: Verify left-to-right directory search order and trailing slash
// normalization. Target: dos_append::find_absolute_path()
...
// Requirement: Verify /PATH:ON searches APPEND list even when path is specified,
// /PATH:OFF bypasses. Target: dos_append::find_absolute_path(), SetFlags()
TEST_F(DosAppendTest, PathOverride)
{
	...
	// Even if we provide an absolute path to a missing file, it should
	// search APPEND!
	dos_append::SetFlags(false, true, false);
	...
	dos_append::SetFlags(false, false, false);
	EXPECT_FALSE(dos_append::find_absolute_path("C:\\OTHER\\README.TXT",
	                                            out_path));
}

// Requirement: Verify /PATH:ON resolves relative, absolute, dot-segment, and
// slash-mixed inputs. Target: dos_append::find_absolute_path()
TEST_F(DosAppendTest, PathOverrideVariants)
{
	...
	EXPECT_TRUE(dos_append::find_absolute_path("..\\SUBDIR\\TARGET.TXT",
	                                           out_path));
	EXPECT_EQ(out_path, "C:\\APPEND_LIB\\TARGET.TXT");

	// 2. Absolute path input
	EXPECT_TRUE(dos_append::find_absolute_path("D:\\OTHER\\DIR\\TARGET.TXT",
	                                           out_path));
	EXPECT_EQ(out_path, "C:\\APPEND_LIB\\TARGET.TXT");
	...
	// 4. Dot segment input
	EXPECT_TRUE(dos_append::find_absolute_path("C:\\DIR\\.\\..\\TARGET.TXT",
	                                           out_path));
	EXPECT_EQ(out_path, "C:\\APPEND_LIB\\TARGET.TXT");
}
...
// Requirement: Verify shell APPEND command correctly parses and sets multiple
// directories. Target: APPEND::Run(), dos_append::SetDirectories()
...
// Requirement: Verify edge-case matrix (quotes, leading semicolons, invalid
// path atomicity). Target: APPEND::Run(), dos_append::validate_directories()
...
// Requirement: Verify DOS_OpenFile hooks search APPEND directories on
// file-not-found errors. Target: DOS_OpenFile(), dos_append::find_absolute_path()
...
// Requirement: Verify that file searching respects the multiplex interrupt B707h
// disable/enable state. Target: DOS_OpenFile(), dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=00h installation check returns AL=FFh.
// Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=02h version check returns AX=FFFFh.
// Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=04h returns ES:DI pointer to DOS memory
// directory list. Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=06h retrieves enabled state in BX.
// Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=07h sets/clears enabled state from BX.
// Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=10h returns version, mode flags, and
// major/minor version. Target: dos_append::MultiplexHandler()
...
// Requirement: Verify unsupported INT 2Fh AH=B7h subfunctions return false.
// Target: dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=03h process sync returns true. Target:
// dos_append::MultiplexHandler()
...
// Requirement: Verify INT 2Fh AH=B7h AL=01h and 11h return true. Target:
// dos_append::MultiplexHandler()
...
// // Requirement: Verify directory paths equal to or exceeding 80 characters
// abort validation. Target: APPEND::Run(), dos_append::validate_directories()
TEST_F(DosAppendTest, ParserPathLengthLimit)
{
	DOS_MakeDir("C:\\GOOD");
	dos_append::SetDirectories("C:\\GOOD");

	// 1. Path exceeding DOS_PATHLENGTH limit (path part excluding drive
...

	{
		...
		// Excessively long path input should abort and preserve
		// previous directory list
		EXPECT_EQ(dos_append::GetDirectories(), "C:\\GOOD");
	}
}

// Requirement: Verify files inside long host directories are correctly resolved
// and opened via their short path aliases. Target: dos_append::ResolvePath(),
// DOS_OpenFile()
TEST_F(DosAppendTest, ResolvePathLongHostDir)
{
	// 1. Create a 76-character long host directory directly on the host
	// filesystem
	std::string host_dir = "tests/files/append/";
	host_dir += std::string(76, 'A');
	std::filesystem::create_directories(host_dir);

	// 2. Create a file inside that directory using the host filesystem
	// directly
...
// Requirement: Verify /X:ON enables wildcard directory searching in
// DOS_FindFirst. Target: DOS_FindFirst(), dos_append::IsExecOn()
TEST_F(DosAppendTest, FindFirstWildcardResolution)
{
	...
	// With /X:ON, FindFirst should resolve *.TXT in C:\\DIR (finding
	// README.TXT)
	...
	// Negative wildcard tests: Ensure wildcards match specifically and
...
// Requirement: Verify /X:ON enables executable path lookup for files in APPEND
// directories. Target: dos_append::IsExecOn(), dos_append::find_absolute_path()
TEST_F(DosAppendTest, ExecModeExecutionSearch)
{
	...
	// /X:OFF (default): Search for executable in APPEND path should be
	// skipped / fail
	dos_append::SetFlags(false, true, false);
	EXPECT_FALSE(dos_append::IsExecOn());
	EXPECT_FALSE(dos_append::IsExecOn() &&
	             dos_append::find_absolute_path("RUN.EXE", resolved_path));
...
// Requirement: Verify /E mode discovers external environment variable edits
// dynamically. Target: dos_append::GetDirectories(), IsEnvOn()
TEST_F(DosAppendTest, EnvModeExternalSetAppend)
{
	...
	// Invoke B704h and verify the returned DOS-memory string reflects the
	// change on demand
	reg_ah       = 0xB7;
	reg_al       = 0x04;
	bool handled = dos_append::MultiplexHandler();
...
// Requirement: Verify /E mode synchronizes directory list changes and clears
// cleanly. Target: dos_append::SetDirectories(), IsEnvOn()
...
// Requirement: Verify B707h disable state overrides active directory list in /E
// mode. Target: dos_append::MultiplexHandler(), IsEnabled()
...
// Requirement: Verify relative option ordering of orthogonal switches has no
// effect on final state. Target: APPEND::Run(), parse_options()
...
// Requirement: Verify illegal/unrecognized switch inputs abort command
// execution without side-effects. Target: APPEND::Run(), parse_options()
