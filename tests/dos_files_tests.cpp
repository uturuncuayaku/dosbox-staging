...
TEST_F(DOS_FilesTest, DOS_Append_Path_Parsing_Nominal)
{
	// Test normal path settings and verification
	DOS_Append::SetPaths("C:\\GAMES;D:\\DATA\\");
	ASSERT_TRUE(DOS_Append::IsActive());
	const auto& paths = DOS_Append::GetPaths();
	ASSERT_EQ(paths.size(), 2);
	EXPECT_EQ(paths[0], "C:\\GAMES");    // verify path without trailing slash remains intact
	EXPECT_EQ(paths[1], "D:\\DATA");     // verify trailing slash was stripped
...
TEST_F(DOS_FilesTest, DOS_Append_Weird_And_Invalid_Inputs)
{
	// 1. Empty and delimiter-only inputs
	DOS_Append::SetPaths("");
	EXPECT_FALSE(DOS_Append::IsActive());
	...
	// 2. Single backslash (verify it does not cause pop_back undefined behavior)
	DOS_Append::SetPaths("\\");
	EXPECT_FALSE(DOS_Append::IsActive());

	// 3. Extra whitespace formatting
	DOS_Append::SetPaths("  C:\\GAMES  ; \t D:\\DATA \r\n ");
	ASSERT_TRUE(DOS_Append::IsActive());
	const auto& paths = DOS_Append::GetPaths();
	ASSERT_EQ(paths.size(), 2);
...
TEST_F(DOS_FilesTest, DOS_Append_Security_And_Boundary_Tests)
{
	// 1. Mount a local drive to C: (index 2) pointing to a test folder
	auto local_drive = create_local_drive("tests/files/paths/");
	Drives.at(2) = local_drive;

	// 2. Path Traversal & Sandbox Escape via MakeName
	char fullname[DOS_PATHLENGTH];
	uint8_t drive;
	
	// Appending a path with traversal dots on mounted drive C: (index 2)
	DOS_Append::SetPaths("C:\\GAMES\\..\\..\\..\\ETC");
	ASSERT_TRUE(DOS_Append::IsActive());
	
	// Resolve it using MakeName
	std::string test_path = DOS_Append::GetPaths()[0] + "\\FILE.TXT";
	bool result = DOS_MakeName(test_path.c_str(), fullname, &drive);
	
	// Verify it resolved safely to C:\ETC\FILE.TXT rather than escaping C:
	EXPECT_TRUE(result);
	EXPECT_EQ(std::string(fullname), "ETC\\FILE.TXT");
	EXPECT_EQ(drive, 2); // C: drive index
	
	// 3. Exact DOS_PATHLENGTH boundaries
	// C:\ + 252 characters = 255 (valid under DOS_PATHLENGTH = 256)
	std::string exact_valid_path = "C:\\" + std::string(252, 'A');
	DOS_Append::SetPaths(exact_valid_path);
	EXPECT_TRUE(DOS_Append::IsActive());
	
	// C:\ + 253 characters = 256 (exactly hits limit / invalid)
	std::string overflow_path = "C:\\" + std::string(253, 'A');
	result = DOS_MakeName(overflow_path.c_str(), fullname, &drive);
	EXPECT_FALSE(result); // Must fail gracefully
	
	// 4. Excessive Delimiters
	// Feed a path string containing 500 consecutive semicolons
	std::string spam_semicolons = std::string(500, ';');
	EXPECT_NO_THROW({
		DOS_Append::SetPaths(spam_semicolons);
	});
	EXPECT_FALSE(DOS_Append::IsActive());
