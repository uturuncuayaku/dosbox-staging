...
bool multiplex_enabled  = true; // Enabled by default (controlled exclusively by
                                // B707h)
...
// Helper 1: Strips leading/trailing quotes and spaces safely
static std::optional<std::string_view> trim_token(std::string_view token)
{
	...
	std::string_view trimmed = token.substr(first, last - first + 1);
	return trimmed.empty() ? std::nullopt
	                       : std::optional<std::string_view>(trimmed);
}

// Helper 2: Validates a single path against DOS drives and normalizes it
static std::optional<std::string> validate_and_normalize_path(std::string_view token)
{
...
std::optional<std::string> validate_directories(std::string_view args)
{
	...
	validated_paths.reserve(args.size());
	...
	while (start < args.size()) {
		...
		if (trimmed) {
			auto normalized = validate_and_normalize_path(*trimmed);
			if (!normalized) {
				return std::nullopt;
			}
...
std::string GetDirectories()
{
	if (env_mode) {
		if (auto shell = DOS_GetFirstShell()) {
			if (shell->psp) {
				if (auto envvar = shell->psp->GetEnvironmentValue(
				            "APPEND")) {
					return *envvar;
				}
			}
...
// Attempts to resolve a FindFirst search pattern against APPEND directories
// when /X is active.
...
bool MultiplexHandler()
{
	...
	case 0x01:
		...
		// MS-DOS aborted the program, but we silently log and succeed
		// to avoid breaking legacy checks.
	...
	case 0x03:
		...
		// DOSBox-Staging is single-tasking, so we handle the call
		// gracefully.
	...
	case 0x06:
		...
		return true;
	case 0x07:
		// Set APPEND state flags from BX. B707h is the only caller
		// allowed to modify multiplex_enabled.
	...
	case 0x10:
		...
		return true;
	case 0x11: {
		// TrueName Support: Resolves ASCIIZ filename from DS:DX via
		// APPEND and canonicalizes into buffer at ES:DI
		std::string resolved = {};
		std::string request  = {};
